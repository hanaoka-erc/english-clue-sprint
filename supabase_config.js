...
function from(){let mode='select',payload=null,date=null;const api={insert(v){mode='insert';payload=v;return api},select(){mode='select';return api},eq(k,v){if(k==='daily_date')date=v;return api},order(){return api},then(resolve,reject){const run=async()=>{try{if(mode==='insert'){const data=await postResult(payload||{});return {data,error:null}}await syncLocalResults();const data=await fetchRows(date||'');return {data,error:null}}catch(error){return {data:null,error}}};run().then(resolve,reject)}};return api}
